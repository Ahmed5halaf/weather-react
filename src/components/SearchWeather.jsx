...
const SearchWeather = () => {
    const [search, setSearch] = useState('londen');
    ...
    useEffect(() => {
        const fetchWether = async () => {
            const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${search}&appid=bd44d3f1cdc6d4097f05f57cdef29dd8`);
            if (componentMounted) {
